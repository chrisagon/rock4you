...
// .wrangler/tmp/bundle-XKFPXa/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-XKFPXa/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
listsRoutes.get("/", zValidator("query", querySchema), async (c) => {
  try {
    ...
    const countQuery = `
      ...
      WHERE ${whereClause}
    `;
    const countResult = await selectFirst(c.env.DB, countQuery, params);
    const total = countResult?.total || 0;
    console.log("Listes retourn\xE9es pour", user.id, ":", lists);
    return c.json({
      success: true,
      data: lists,
      pagination: {
